...
int main (void)
{
	...
	for (fils = 0; fils < NB_FILS; fils ++) {
		...
		if (pid != 0) // c'est le pid du pere
			continue;
		gentillesse = fils * (20 / (NB_FILS - 1));
		if (nice(gentillesse) < 0) { // on declare la gentillesse au noyau
			perror("nice");
			exit(EXIT_FAILURE);
		}
		/* attente signal de demarrage */
		pause(); // etat stoppe, en attente d'un signal
		/* comptage */
		while (1)
			compteur ++;
	}
	/* processus pere */
	/* Modification du gestionnaire de signal
	   le pere s'immunise face a SIGUSR1 */
	signal(SIGUSR1, SIG_IGN);
	sleep(1);
	kill(-getpgid(0), SIGUSR1);
	sleep(5);
	kill(-getpgid(0), SIGUSR1);
	while (wait(NULL) > 0);    // Le pere recupere la valeur de retour des fils
