...
for (i=0;i<nb_octets;i++) printf ("%c",buffer [1] [i]);
printf("\n");
...
thread (sigval_t valeur)
{
	...
	if (aio_error (cb) == EINPROGRESS)
		return;
	nb_octets = aio_return (cb);
for (i=0;i<nb_octets;i++) printf ("%c",buffer [2] [i]);
 printf("\n");
...
main (int argc, char * argv [])
{
	...
	cb [2] . aio_nbytes   = 256;
	cb [2] . aio_reqprio  = 0; // valeur retirée à la priorité du thread si il est SCHED_RR  ou SHCED_FIFO autrement dit quand il est temps réel
	...
	cb [2] . aio_sigevent . sigev_notify_attributes = NULL; // Atrributs du nouveua thread " Joignable vs détaché ; Scheduling : FIFO;RR;OTHER;" ici valeurs par défaut
	cb [2] . aio_sigevent . sigev_value  . sival_ptr = & cb [2]; // On passe au thread l'adresse de la structure que le noyau a utilisé
	...
	fprintf (stdout, "Lectures lancées\n");// On confirme que le noyau travail est que le processus continue son execution
	while ((aio_error (& cb [0]) == EINPROGRESS) // Toute les secondes, on verifie que le noyau n'a produit aucune erreur sur chacune des lectures asynchrones
	    || (aio_error (& cb [1]) == EINPROGRESS)		
	    || (aio_error (& cb [2]) == EINPROGRESS))
		sleep (1);
	nb_octets = aio_return (& cb [0]); // on verifie que tous les blocs ont bien été lus pour le cas a) car pas de notification
